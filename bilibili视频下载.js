...
!function() {
    ...
    var clazzMap = {}, VideoBase = function() {
        ...
        return video_base_createClass(Video, [ {
            key: "total",
            value: function total() {
                return this.epList.length ? this.video_list.length : this.state.videoData.pages.length;
            }
        }, {
            key: "title",
            value: function title(p) {
                return this.epList.length && p ? this.video_list[this.id(p)].title : this.state.videoData.pages[this.id(p)].part;
            }
        }, {
            key: "filename",
            value: function filename(p) {
                if (this.epList.length && p) return this.title(p).replace(/[\/\\:*?"<>|]+/g, "");
            ...
            key: "getName",
            value: function getName() {
                return this.epList.length ? this.state.sectionsInfo.title : _get(_getPrototypeOf(Video.prototype), "getName", this).call(this);
            }
        }, {
            key: "aid",
            value: function aid(p) {
                return this.epList.length && p ? this.video_list[this.id(p)].aid : this.state.aid || this.state.videoData.aid;
            }
        }, {
            key: "bvid",
            value: function bvid(p) {
                return this.epList.length && p ? this.video_list[this.id(p)].bvid : this.state.bvid || this.state.videoData.bvid;
            }
        }, {
            key: "cid",
            value: function cid(p) {
    ...
    function common_slicedToArray(r, e) {
        return function common_arrayWithHoles(r) {
            if (Array.isArray(r)) return r;
        }(r) || function common_iterableToArrayLimit(r, l) {
            var t = null == r ? null : "undefined" != typeof Symbol && r[Symbol.iterator] || r["@@iterator"];
            if (null != t) {
                var e, n, i, u, a = [], f = !0, o = !1;
                try {
        ...
        }(r, e) || function common_unsupportedIterableToArray(r, a) {
            if (r) {
                if ("string" == typeof r) return common_arrayLikeToArray(r, a);
                var t = {}.toString.call(r).slice(8, -1);
                return "Object" === t && r.constructor && (t = r.constructor.name), "Map" === t || "Set" === t ? Array.from(r) : "Arguments" === t || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(t) ? common_arrayLikeToArray(r, a) : void 0;
            }
        }(r, e) || function common_nonIterableRest() {
            throw new TypeError("Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.");
        }();
    }
    function common_arrayLikeToArray(r, a) {
        (null == a || a > r.length) && (a = r.length);
        for (var e = 0, n = Array(a); e < a; e++) n[e] = r[e];
        return n;
    }
    ...
    function common_defineProperty(e, r, t) {
        return (r = function common_toPropertyKey(t) {
            var i = function common_toPrimitive(t, r) {
                if ("object" != common_typeof(t) || !t) return t;
                var e = t[Symbol.toPrimitive];
                if (void 0 !== e) {
                    var i = e.call(t, r || "default");
                    if ("object" != common_typeof(i)) return i;
                    throw new TypeError("@@toPrimitive must return a primitive value.");
                }
                return ("string" === r ? String : Number)(t);
            }(t, "string");
            return "symbol" == common_typeof(i) ? i : i + "";
    ...
    function getSignData(param, sec) {
        return common_objectSpread(common_objectSpread({}, param), {}, {
            sign: md5("".concat(Object.entries(param).filter(function(_ref2) {
                return "sign" !== common_slicedToArray(_ref2, 1)[0];
            }).map(function(e) {
    ...
    function checkMotrixRunning(callback) {
        ...
        }, controller = new AbortController, id = setTimeout(function() {
    ...
    function handleMotrixNotRunning(execDownload) {
        var startTime = Date.now(), pollTimer = null, hasDownloaded = !1;
        function stopPolling() {
            pollTimer && (clearInterval(pollTimer), pollTimer = null);
        }
        function startPolling() {
            pollTimer = setInterval(function() {
                if (Date.now() - startTime > 1e4) return stopPolling(), void (hasDownloaded || message_Message_error("连接超时：Motrix 启动失败或未响应"));
                checkMotrixRunning(function(running) {
                    running && !hasDownloaded && (!function sendDownload() {
                        hasDownloaded || (execDownload && execDownload(), hasDownloaded = !0), stopPolling();
                    }(), message_Message_success("已连接到 Motrix，任务已发送"));
                });
            }, 500);
        }
        if ("1" === config_config.show_motrix_confirm_open_auto) return window.open("motrix://"), 
        void startPolling();
        MessageBox_confirm("未检测到运行的 Motrix，是否启动？", function() {
            window.open("motrix://"), startPolling();
        }, function() {
            (function cancelDownload() {
                stopPolling(), message_Message_info("下载已取消");
            })();
        });
    }
    function checkMotrixRunningBeforeExecute(execFunc) {
        checkMotrixRunning(function(running) {
            running ? execFunc && execFunc() : handleMotrixNotRunning(execFunc);
        });
    }
    ...
    function open_ariang(rpc) {
        var hash_tag = rpc ? get_ariang_set_hash(rpc) : "", a = document.createElement("a");
        return a.style.display = "none", a.onclick = function() {
            window.bp_aria2_window = window.open(config_config.ariang_host), hash_tag && setTimeout(function() {
                window.bp_aria2_window.location.href = config_config.ariang_host + hash_tag;
            }, 500);
        }, document.body.appendChild(a), a.click(), document.body.removeChild(a), window.bp_aria2_window;
    }
    function download_rpc_ariang() {
        ...
        0 != videos.length && (1 == videos.length && Array.isArray(videos[0]) ? download_rpc_ariang.apply(void 0, download_toConsumableArray(videos[0])) : (!function download_rpc_ariang_send(video) {
            ...
            bp_aria2_window && !bp_aria2_window.closed || (open_ariang(), time = 3e3), setTimeout(function() {
                ...
                (video.rpc_dir || config_config.ariang_dir) && cmd.push("dir=".concat(encodeURIComponent(video.rpc_dir || config_config.ariang_dir)));
    ...
    var _document$head$innerH, Download = {
        ...
        download: function download(url, filename, type) {
            ...
            "blob" === type ? download_blob(url, filename) : "rpc" === type ? function download_rpc(url, filename, rpc_dir) {
                var type = arguments.length > 3 && void 0 !== arguments[3] ? arguments[3] : "post";
                if ("post" === type) {
                    if ("0" === config_config.show_motrix_confirm) return void download_rpc_post({
                        url: url,
                        filename: filename,
                        rpc_dir: rpc_dir
                    });
                ...
                } else "ariang" === type && download_rpc_ariang({
                    url: url,
                    filename: filename,
                    rpc_dir: rpc_dir
                });
            }(url, filename, null, rpc_type()) : "api" === type && function download_api(url, filename) {
                var api = config_config.download_api, api_sec = config_config.download_api_secret;
                if (api) {
                    var data = {
                        filename: filename,
                        ts: Date.now(),
                    ...
                    }), message_Message_success("请求已发送");
                } else message_Message_warning("请配置下载接口");
            }(url, filename);
        },
        ...
        download_all: function download_all() {
            ...
            MessageBox_confirm(msg, function() {
                ...
                (dl_video || dl_audio) && ("post" === rpc_type() && "0" !== config_config.show_motrix_confirm ? checkMotrixRunningBeforeExecute(function() {
                    download_videos([].concat(video_tasks));
                }) : download_videos([].concat(video_tasks))), dl_subtitle && (1 === video_tasks.length ? download_subtitle_vtt(video_tasks[0].p, video_tasks[0].filename) : download_subtitle_vtt_zip([].concat(video_tasks), new JSZip)), 
            ...
            if (dl_format_change_evt(config_config.format), $("body").on("change", "#dl_format", function() {
                var format = $(this).val();
            ...
            }), "0" !== config_config.video_list_auto_scroll_load) {
                var _$$, h = null === (_$$ = $("#playlist-video-action-list")[0]) || void 0 === _$$ ? void 0 : _$$.scrollHeight;
                if (!h) return;
                var timer1 = null, timer2 = null, old_h = h, change_count = 0, not_change_count = 0, load_down = !0;
                timer1 = setInterval(function() {
                    load_down ? $("#playlist-video-action-list").scrollTop($("#playlist-video-action-list")[0].scrollHeight) : $("#playlist-video-action-list").scrollTop(0), 
                    load_down = !load_down, not_change_count > 12 && clearInterval(timer1);
                }, 1500), timer2 = setInterval(function() {
                    (h = $("#playlist-video-action-list")[0].scrollHeight) > old_h ? (!change_count && MessageBox_confirm("正在加载视频列表，请稍后...", null, function() {
                        clearInterval(timer1), clearInterval(timer2);
                    }), change_count++, not_change_count = 0) : not_change_count++, old_h = h, not_change_count > 12 && (clearInterval(timer1), 
                    clearInterval(timer2), change_count > 0 && (message_Message_info("视频列表加载结束"), download_all()));
                }, 500);
            }
        },
    ...
    var config_config = {
        ...
        ariang_sync_config: function ariang_sync_config() {
            var rpc = {
                domain: config_config.rpc_domain,
                port: config_config.rpc_port,
                path: config_config.rpc_path,
                token: config_config.rpc_token
            }, ariang_window = window.bp_aria2_window;
            if (ariang_window && !ariang_window.closed) return ariang_window.location.href = config_config.ariang_host + Download.get_ariang_set_hash(rpc), 
            void message_Message_info("已同步配置");
    ...
    var video_download_all_rpc_confirm = !0;
    var event_event = {
        ...
        video_download_all: function video_download_all() {
            user.lazyInit(!0), "local" === config_config.request_type || auth.hasAuth() ? "rpc" !== config_config.download_type && video_download_all_rpc_confirm ? MessageBox_confirm("仅支持使用RPC接口批量下载，请确保RPC环境正常，是否继续？", function() {
                Download.download_all(), video_download_all_rpc_confirm = !1;
            }) : Download.download_all() : MessageBox_confirm("批量下载仅支持本地请求或授权用户使用RPC接口下载，是否进行授权？", function() {
                auth.login();
            });
        },
    ...
    var Main = function() {
        ...
        return function main_createClass(e, r, t) {
            ...
            value: function loadToolbar() {
                var timer, timeout, loading = !1;
                timer = setInterval(function() {
                    document.getElementById("nav-searchform") && (loading || (loading = !0, initToolbar()), 
                    clearInterval(timer));
                }, 500);
                try {
                    timeout = (timeout = config_config.show_ui_timeout ? parseInt(config_config.show_ui_timeout) : 8) > 0 ? timeout : 8;
                } catch (err) {
                    console.error("show_ui_timeout err:", err);
                }
                setTimeout(function() {
                    if (clearInterval(timer), !loading) {
                        if (console.warn("waiting timeout..."), "1" === config_config.show_ui_confirm) return "1" === config_config.show_ui_confirm_load_force ? (loading = !0, 
                        void initToolbar()) : void MessageBox_confirm("加载脚本工具栏超时，建议刷新页面重新加载，是否强制加载工具栏？", initToolbar, null);
                        message_Message_warning("工具栏加载异常，已延迟加载"), setTimeout(function() {
                            loading || (loading = !0, initToolbar()), message_Message_info("工具栏已重新加载");
                        }, 5e3);
                    }
                }, 1e3 * timeout);
            }
