...
(function () {
	...
	const websites = [
		...
		},
		{
			...
			fun: function () {
				loginObserver(".login-btn", ".icon-btn-wrapper");
			},
		},
		{
			name: "喜马拉雅",
		...
		},
		{
			...
			items: [],
			fun: function () {
				loginObserver(".ant-btn.btn.login.ant-btn-primary", ".ant-modal-close");
			},
		},
	];
	 ...
	 * 登录弹窗只弹出一次时的处理方案
	 *
	 * @param      {string}  unLoginBtn  The un login button class
	 * @param      {string}  closeBtn    The close button class
	 */
	function loginObserver(unLoginBtn, closeBtn) {
		let mo = new MutationObserver(function (mutations) {
			let unloginBtn = document.querySelector(unLoginBtn);
			if (unloginBtn) {
				for (let mutation of mutations) {
					for (let node of mutation.addedNodes) {
						let button = node.querySelector(closeBtn);
						if (button) {
							console.log("关闭成功");
							button.click();
							mo.disconnect();
